...
//            exit;
                ...
                if($_POST['type']=='1'){
                    ...
                    if(!empty(input("file.pic"))){
                        ...
                        $path_1 = 'public' . DS . 'img' ;
                        $file_img->move(ROOT_PATH . $path_1,$imgname ); //保存文件
                        Db::table('guanggao')->where('g_id',$_GET['g_id'])->update(['g_name'=>$name,'g_info'=>$info,'g_type'=>$type,'picname'=>$imgname,'plan_id'=>$plan_id,'status'=>$status,'g_lianjie_A' => $lianjie_A,'g_lianjie_I' => $lianjiename2]);
                        Guang::corejs($plan_id);
                        return $this->success('修改成功','Guanggao/index');
                    }else{
                        return $this->error('修改失败');
...
//            exit;
            ...
            $type = Guang::getGuanggaoById($g_id);
             $data = array(
                'module_name' => '管理员',
                'module_url' => SITE_URL . '/admin/guanggao/',
                'module_slug' => 'guanggao',
